...
RTC_KERNEL_DEFCONFIG := odroidc_rtc_shield_defconfig

KERNEL_ROOTDIR := kernel
KERNEL_OUT := $(TARGET_OUT_INTERMEDIATES)/KERNEL_OBJ
RTC_KERNEL_OUT := $(TARGET_OUT_INTERMEDIATES)/RTCKERNEL_OBJ
KERNEL_CONFIG := $(KERNEL_OUT)/.config
RTC_KERNEL_CONFIG := $(RTC_KERNEL_OUT)/.config
KERNEL_MODULES_INSTALL := system
KERNEL_MODULES_OUT := $(TARGET_OUT)/lib/modules
TARGET_PREBUILT_KERNEL := $(KERNEL_OUT)/arch/arm/boot/uImage
RTC_TARGET_PREBUILT_KERNEL := $(RTC_KERNEL_OUT)/arch/arm/boot/uImage
...
$(RTC_KERNEL_OUT):
	mkdir -p $(RTC_KERNEL_OUT)

$(KERNEL_CONFIG): $(KERNEL_OUT)
	$(MAKE) -C $(KERNEL_ROOTDIR) O=../$(KERNEL_OUT) ARCH=arm CROSS_COMPILE=$(PREFIX_CROSS_COMPILE) $(KERNEL_DEFCONFIG)
	$(MAKE) -C $(KERNEL_ROOTDIR) O=../$(RTC_KERNEL_OUT) ARCH=arm CROSS_COMPILE=$(PREFIX_CROSS_COMPILE) $(RTC_KERNEL_DEFCONFIG)

$(TARGET_PREBUILT_KERNEL): $(KERNEL_OUT) $(RTC_KERNEL_OUT) $(KERNEL_CONFIG)
	@echo " make uImage"
	$(MAKE) -C $(KERNEL_ROOTDIR) O=../$(KERNEL_OUT) ARCH=arm CROSS_COMPILE=$(PREFIX_CROSS_COMPILE) uImage
	$(MAKE) -C $(KERNEL_ROOTDIR) O=../$(RTC_KERNEL_OUT) ARCH=arm CROSS_COMPILE=$(PREFIX_CROSS_COMPILE) uImage
	...
	$(mv-modules)
	$(clean-module-folder)
	cp $(RTC_TARGET_PREBUILT_KERNEL) $(TARGET_OUT)/etc/Image_rtcshield
